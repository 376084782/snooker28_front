...
export default class AudioPlayer {
  ...
  public static setEffectVolume(value: number): void {
    ...
    this._effect.forEach((clip, id) =>
      cc.audioEngine.setVolume(id, realVolume)
    );
  }
   ...
   * @param loop 循环
   */
  public static async playMusicByUrl(url) {
    let clip = (await Utils.load(url, cc.AudioClip)) as cc.AudioClip;
    this.playMusic(clip);
  }
  /**
   * 播放音乐
   * @param clip 音频
   */
  public static async playMusic(clip: cc.AudioClip) {
    if (this._music.has(clip)) this.stopMusic(clip);
    let id = cc.audioEngine.play(
      clip,
      true,
   ...
   * @param loop 循环
   */
  public static async playEffectByUrl(url, loop = false) {
    let clip = (await Utils.load(url, cc.AudioClip)) as cc.AudioClip;
  ...
  public static playEffect(clip: cc.AudioClip, loop: boolean): void {
    let id = cc.audioEngine.play(
      clip,
      loop,
      this._masterVolume * this._effectVolume
    );
  ...
  public static pauseEffect(clip: cc.AudioClip): void {
    this._effect.forEach(
      (_clip, id) => _clip === clip && cc.audioEngine.pause(id)
    );
  }
  ...
  public static resumeEffect(clip: cc.AudioClip): void {
    this._effect.forEach(
      (_clip, id) => _clip === clip && cc.audioEngine.resume(id)
    );
  }
