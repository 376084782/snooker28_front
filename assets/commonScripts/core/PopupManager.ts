...
export default class PopupManager {
  ...
  public static get current() {
    return this.openedList[this.openedList.length - 1];
  }
  ...
  public static show<Options>(
    ...
    return new Promise(async res => {
      let objSame = this.openedList.find(e => e.path == path);
      if (objSame) {
        if (JSON.stringify(objSame.options) == JSON.stringify(options)) {
          console.log("已经有");
          return;
        }
      }
      ...
      // 当前已有弹窗在展示中则加入等待队列
      // AudioPlayer.playEffectByUrl("sound/弹窗音效", false);

      // 保存为当前弹窗，阻止新的弹窗请求
      this.openedList.push({ path, options, mode });
  ...
  public static hideCurrent() {
    ...
    if (this.current.popup) {
      this.current.popup.hide();
    } else if (this.current.node) {
      this.current.node.destroy();
      this.release(this.current.path);
      this.openedList.pop();
    }
  }
  ...
  private static next(): void {
    if (this.current || this._queue.length === 0) {
      return;
    }
